    ...
    try:
        if request.method == 'POST':
            ...
            if response.status_code == 200:
                ...
                return render(request, 'documentos/documento.html', {'mensaje': mensaje, 'registro_temp':registro_temp,'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario()})
            else:
                mensaje = data['message']
                logger = definir_log_info('error_crear','logs_documento')
            ...
            return render(request, 'documentos/documento.html',{'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario()})
...
def abrir_actualizar_documentos(request):
    try:
        if request.method == 'POST':
            ...
            context = {'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'documentos': documentos, 'mensaje':mensaje}
            mensaje = data['message']
            return render(request, 'documentos/documentoactualizar.html', context)
    except Exception as e:
        logger = definir_log_info('excepcion_documento','logs_documento')
        logger.exception("Ocurrio una excepcion:" + str(e))
        context = {'docuemnto': documentos, 'mensaje':mensaje,'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario()}
        return render(request, 'documentos/documentoactualizar.html', context)
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if rsp['message'] == "La actualización fue exitosa.":
                ...
                return render(request, 'documentos/documentoactualizar.html', {'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'mensaje': mensaje,'documentos':documentos })
            ...
            if response.status_code == 200:
                ...
                return render(request, 'documentos/documentoactualizar.html', {'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'documentos': documentos})
            else:
                mensaje = data['message']
                logger = definir_log_info('error_actualizar','logs_documento')
                logger.warning("Se obtuvo una respuesta invalida: " + mensaje)
        ...
        logger = definir_log_info('excepcion_documento','logs_documento')
        logger.exception("Ocurrio una excepcion:" + str(e))
        return render(request, 'documentos/documentoactualizar.html', {'mensaje': mensaje,'documentos':documentos,'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario()})
...
def eliminar_documento(request, id):
    try:
        ...
        context = {'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'documentos': documentos, 'error': mensaje}
        return render(request, 'documentos/buscarDocumento.html', context)     
...
def buscar_documentos(request):
    try:    
        ...
        if valor is not None and (len(valor)>0):
            ...
            if valor.isdigit():
                ...
                if response.status_code == 200:
                    ...
                    return render(request, 'documentos/buscarDocumento.html', {'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'documentos': documentos, 'mensaje': mensaje})
        ...
        if response.status_code == 200:
            ...
            logger = definir_log_info('buscar_documento','logs_documento')
            logger.debug("Se obtuvieron todos los documentos: " + mensaje ) 
            return render(request, 'documentos/buscardocumento.html', {'documentos': documentos, 'mensaje': mensaje,'reportes_lista':DatosReportes.cargar_lista_documentos(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario()})
        else:
            documentos = []
            mensaje = 'No se encontraron documentos'
            logger = definir_log_info('error_buscar_documento','logs_documento')
