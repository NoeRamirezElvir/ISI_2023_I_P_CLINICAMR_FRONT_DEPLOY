...
def eliminar_precio_historico_tratamiento(request, id):
    try:
        if request.method == 'POST':
            ...
            mensaje = res['message']
            context = {'reportes_lista':DatosReportes.cargar_lista_historico_precio_tratamiento(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'historicos': historicos, 'mensaje': mensaje}
        ...
        context = {'reportes_lista':DatosReportes.cargar_lista_historico_precio_tratamiento(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'historicos': historicos, 'error': mensaje}
        return render(request, 'precio_historico_tratamiento/buscar_precio_historico_tratamiento.html', context)     
        ...
        if valor is not None and (len(valor)>0):      
            if valor.isdigit():
                ...
                if response.status_code == 200:
                    ...
                    logger = definir_log_info('buscar_precio_historico_tratamiento','logs_precio_historico_tratamiento')
                    logger.info(f"No se obtuvieron los registros: Filtrado(ID){valor} - {mensaje}")
                    return render(request, 'precio_historico_tratamiento/buscar_precio_historico_tratamiento.html', {'reportes_lista':DatosReportes.cargar_lista_historico_precio_tratamiento(),'datos_permisos':cargar_datos(),'reportes_usuarios':DatosReportes.cargar_usuario(),'historicos': historicos, 'mensaje': mensaje})
