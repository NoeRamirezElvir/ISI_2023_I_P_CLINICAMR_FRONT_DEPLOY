...
function corregirDatosHidden() {
  ...
  document.getElementById("exportarBtn").addEventListener("click", function() {
    ...
    var datosManipulados = listaDatos.map(function(dato) {
      return {
        ID: dato.id,
        Paciente: dato.idMuestra.idPaciente.documento + "/" + dato.idMuestra.idPaciente.nombre,
        Tipo: dato.idTipo.nombre,
        Laboratorio: dato.idLaboratorio.nombre.nombre,
  ...
  document.getElementById("exportBtn").addEventListener("click", function() {
    ...
    var docDefinition = {
      content: [
        ...
        { text: 'Reporte de Examenes', style: 'header', alignment: 'center' },
        { text: '\n' },
        ...
        { text: '\n' },
        {
          table: {
            headerRows: 1,
            widths: [15, 90, 90, 90,90,90],
            body: [
              [
                { text: 'ID', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Paciente', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Tipo', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Laboratorio', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Fecha Programada', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Observación', style: 'tableHeader', fillColor: '#C6C6C6' }
              ],
              ...datos.map(function(dato, index) {
                return [
                  { text: dato.id, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.idMuestra.idPaciente.documento + "/" + dato.idMuestra.idPaciente.nombre, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.idTipo.nombre, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.idLaboratorio.nombre, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.fechaProgramada, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.observacion, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' }
                ];
              })
            ]
          }
