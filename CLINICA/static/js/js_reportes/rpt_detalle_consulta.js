...
document.getElementById("exportarBtn").addEventListener("click", function() {
  ...
  var datosManipulados = listaDatos.map(function(dato) {
      ...
      if (dato.idDiagnostico.idEnfermedades) {
        for (var clave in dato.idDiagnostico.idEnfermedades) {
          if (dato.idDiagnostico.idEnfermedades.hasOwnProperty(clave)) {
            enfermedadNombre += dato.idDiagnostico.idEnfermedades[clave].nombre + ', ';
          }
        }
        // Eliminar la última coma y espacio
        enfermedadNombre = enfermedadNombre.slice(0, -2);
      }      
      return {
        ID: dato.id,
        Paciente: dato.idConsulta.paciente,
        Diagnostico: dato.idDiagnostico.descripcion,
  ...
  var encabezados = [
    ["Reporte de Detalles de Consultas"],
...
document.getElementById("exportBtn").addEventListener("click", function() {
  ...
  var docDefinition = {
    content: [
      ...
      { text: 'Reporte de Detalles de Consultas', style: 'header', alignment: 'center' },
      { text: '\n' },
      ...
      { text: '\n' },
      {
        table: {
          headerRows: 1,
          widths: [20, 150, 150, 150],
          body: [
            [
              { text: 'ID', style: 'tableHeader', fillColor: '#C6C6C6' },
              { text: 'Paciente', style: 'tableHeader', fillColor: '#C6C6C6' },
              { text: 'Diagnóstico', style: 'tableHeader', fillColor: '#C6C6C6' },
              { text: 'Enfermedad', style: 'tableHeader', fillColor: '#C6C6C6' }
            ],
            ...datosManipulados.map(function(dato, index) {
              return [
                { text: dato.ID, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9', style: 'name' },
                { text: dato.Paciente, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9', style: 'name' },
                { text: dato.Diagnostico, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9', style: 'name' },
                { text: dato.Enfermedad, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9', style: 'name' }
              ];
            })
          ]
        }
