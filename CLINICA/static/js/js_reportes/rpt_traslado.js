...
function corregirDatosHidden() {
  ...
  document.getElementById("exportarBtn").addEventListener("click", function() {
    ...
    var datosManipulados = listaDatos.map(function(dato) {
      return {
        ID: dato.id,
        Paciente: dato.idPaciente.documento + "/" + dato.idPaciente.nombre,
        Motivos: dato.idAutorizacionPaciente.motivos,
        Empleado: dato.idEmpleado.nombre + " " + dato.idEmpleado.apellidos,
        LugarTraslado: dato.nombre,
    ...
    var encabezados = [
      ["Reporte de Traslado de Pacientes"],
  ...
  document.getElementById("exportBtn").addEventListener("click", function() {
    ...
    var docDefinition = {
      content: [
        ...
        { text: 'Reporte de Traslado de Pacientes', style: 'header', alignment: 'center' },
        { text: '\n' },
          ...
          table: {
            ...
            body: [
              [
                ...
                { text: 'Motivos', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Empleado', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Lugar de Traslado', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Teléfono', style: 'tableHeader', fillColor: '#C6C6C6' },
                { text: 'Dirección', style: 'tableHeader', fillColor: '#C6C6C6' }
              ],
              ...datos.map(function(dato, index) {
                return [
                  { text: dato.id, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.idPaciente.documento + "/" + dato.idPaciente.nombre, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.idAutorizacionPaciente.motivos, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.idEmpleado.nombre + " " + dato.idEmpleado.apellidos, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.nombre, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.telefono, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' },
                  { text: dato.direccion, fillColor: index % 2 === 0 ? '#E5E5E5' : '#D9D9D9',style:'name' }
                ];
              })
            ]
          }
